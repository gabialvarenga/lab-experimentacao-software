# Laboratório de Experimentação de Software

Repositório do grupo para os laboratórios da disciplina de Laboratório de
Experimentação de Software (PUC Minas), sob orientação do Prof. Danilo Maia.

## Integrantes

- Brenda Evers (1523565)
- Carlos José Gomes Batista Figueiredo (1507022)
- Gabriela Alvarenga Cardoso (1026227)

## GitHub Projects (Kanban)

Board do grupo, usado em todos os laboratórios do semestre:
[Kanban](https://github.com/users/gabialvarenga/projects/9)

## Laboratórios

Cada laboratório vive na sua própria pasta, com README, dependências e
configuração próprias.

| Lab | Descrição | Status |
|---|---|---|
| [lab01/](lab01/) | Características de repositórios populares do GitHub + setup do Kanban | Concluído |
| [lab02/](lab02/) | Assistentes de IA vs. codificação manual: experimento controlado | Em andamento |
| lab03/ | — | Ainda não iniciado |
| lab04/ | — | Ainda não iniciado |
| lab05/ | — | Ainda não iniciado |
