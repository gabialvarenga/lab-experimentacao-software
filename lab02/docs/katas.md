...
> **Atualização — de volta às 6 katas:** por causa do prazo curto, o grupo
> havia reduzido temporariamente para 4 katas (k1..k4). O professor exigiu
> as 6 katas completas, então k5 e k6 foram restauradas do histórico do
> repositório (eram as antigas "k3 — mesclagem de leituras" e "k6 —
> ranking de trilhas" da seleção original desta issue). Os quatro trials já
> executados sob o desenho reduzido (Gabriela, ordens 1–4) continuam válidos
> — só as ordens 5–6 (k5, k6) ficam pendentes para os três integrantes.
...
| k2 | `validar_lote` | 11 | 5 | validação de formato, três checagens acumuladas em ordem fixa |
| k3 | `montar_escala` | 10 | 6 | atribuição gulosa com desempate, filtro de disponibilidade, um erro |
| k4 | `calcular_tarifa` | 11 | 5 | parsing de `HH:MM`, franquia, cobrança por faixa, teto, dois erros |
| k5 | `mesclar_leituras` | 10 | 6 | junção de listas, dedup com desempate, ordenação, imutabilidade |
...
agregar, formatar). k1 e k2 são as mais simples do grupo, k4 a que exige mais
...
| 2 | Tarifa de estacionamento por hora iniciada, com teto | **selecionado (k4)** |
| 3 | Mesclagem de leituras de dois sensores com desempate por precisão | **selecionado (k5)** |
| 4 | Escala de plantão com indisponibilidade e balanceamento de carga | **selecionado (k3)** |
| 5 | Validação de código de lote com dígito verificador | **selecionado (k2)** |
...
com IA e três sem. A ordem das katas é rotacionada entre os integrantes para
que nenhuma kata caia sempre no mesmo ponto da sequência, controlando o
efeito de aprendizado. As ordens 1–4 de Brenda, Carlos e Gabriela já vinham
do desenho reduzido (4 katas) e foram mantidas; as ordens 5–6 (k5, k6) são a
extensão para fechar as 6.
...
| Carlos | k2, k4, k5 | k1, k3, k6 |
| Gabriela | k1, k4, k6 | k2, k3, k5 |
...
alternada entre katas fecha o balanço no total do grupo.
